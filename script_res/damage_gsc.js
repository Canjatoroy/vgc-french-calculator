...
function CALCULATE_DAMAGE_GSC(attacker, defender, move, field) {
    ...
    if (move.name === "Fléau" || move.name === "Contre") {
        move.isCrit = false;
    ...
    if (ignoreMods) {
        ...
        if (isPhysical && attacker.status === "Brûlé") {
            at = Math.floor(at / 2);
    ...
    if ((attacker.name === "Pikachu" && attacker.item === "Balle Lumière" && !isPhysical) ||
            ((attacker.name === "Osselait" || attacker.name === "Ossatueur") && attacker.item === "Masse Os" && isPhysical)) {
        at *= 2;
    ...
    if (defender.name === "Métamorph" && defender.item === "Poudre Métal") {
    ...
    } else if ((field.weather === "Soleil" && move.type === "Eau") || (field.weather === "Pluie" && (move.type === "Feu" || move.name === "Lance-Soleil"))) {
