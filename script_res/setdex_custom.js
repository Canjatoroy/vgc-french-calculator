var SETDEX_CUSTOM = {};
var showdownFormes = [["Kyurem-Blanc", "Kyurem-B"],
["Kyurem-Noir", "Kyurem-N"],
["Motisma-Lavage", "Motisma-L"],
["Motisma-Chaleur", "Motisma-C"],
["Motisma-Froid", "Motisma-F"],
["Motisma-Tonte", "Motisma-T"],
["Motisma-Hélice", "Motisma-H"],
["Giratina-Origine", "Giratina-O"],
["Démétéros-Totémique", "Démétéros-T"],
["Fulguris-Totémique", "Fulguris-T"],
["Boréas-Totémique", "Boréas-T"],
["Floette-Éternel", "Floette-É"],
["Pitrouille", "Pitrouille-Moyen"],
["Banshitrouye", "Banshitrouye-Moyen"],
["Cheniselle-Sable", "Cheniselle-S"],
["Cheniselle-Déchet", "Cheniselle-D"],
["Primo-Groudon", "Groudon"],
["Primo-Kyogre", "Kyogre"]];
...
var savecustom = function()
{
	...
	if(spreadName == '')
		spreadName = "Mon Set Personnalisé";
		...
		for(var i = 2; i < lines.length; ++i){
			...
			if(lines[i].indexOf("EVs") != -1) //if EVs are in this line
			{
				...
				for(var j = 0; j < evList.length; ++j){
					...
					if(evListElements[1] == "PV")
						EVs[0] = parseInt(evListElements[0])
					else if(evListElements[1] == "Attaque")
						EVs[1] = parseInt(evListElements[0])
					else if(evListElements[1] == "Défense")
						EVs[2] = parseInt(evListElements[0])
					else if(evListElements[1] == "Atk.Spé")
						EVs[3] = parseInt(evListElements[0])
					else if(evListElements[1] == "Déf.Spé")
						EVs[4] = parseInt(evListElements[0])
					else if(evListElements[1] == "Vitesse")
			...
			if(lines[i].indexOf("IVs") != -1) //if EVs are in this line
			{
				...
				for(var j = 0; j < ivList.length; ++j){
					...
					if(ivListElements[1] == "PV")
						IVs[0] = parseInt(ivListElements[0])
					else if(ivListElements[1] == "Attaque")
						IVs[1] = parseInt(ivListElements[0])
					else if(ivListElements[1] == "Défense")
						IVs[2] = parseInt(ivListElements[0])
					else if(ivListElements[1] == "Atk.Spé")
						IVs[3] = parseInt(ivListElements[0])
					else if(ivListElements[1] == "Déf.Spé")
						IVs[4] = parseInt(ivListElements[0])
					else if(ivListElements[1] == "Vitesse")
