...
function autoSetTerrain()
{
    var ability1 = $("#p1 .ability").val()
    var ability2 = $("#p2 .ability").val()
    if((ability1 == "Créa-Élec" || ability2 == "Créa-Élec")){
        $("input:radio[id='electric']").prop("checked", true)
        lastTerrain = 'electric';
    }
    else if((ability1 == "Créa-Herbe" || ability2 == "Créa-Herbe")){
        $("input:radio[id='grassy']").prop("checked", true)
        lastTerrain = 'grassy';
    }
    else if((ability1 == "Créa-Brume" || ability2 == "Créa-Brume")){
        $("input:radio[id='misty']").prop("checked", true)
        lastTerrain = 'misty';
    }
    else if((ability1 == "Créa-Psy" || ability2 == "Créa-Psy")){
...
function autosetWeather(ability, i) {
    ...
    var primalWeather = ["Terre Finale", "Mer Primaire"];
    var autoWeatherAbilities = {
            "Sécheresse": "Soleil",
            "Crachin": "Pluie",
            "Sable Volant": "Sable",
            "Alerte Neige": "Grêle",
            "Terre Finale": "Terre Finale",
            "Mer Primaire": "Mer Primaire",
            "Souffle Delta": "Souffle Delta"
        };
    var newWeather;

    if (ability in autoWeatherAbilities) {
        lastAutoWeather[i] = autoWeatherAbilities[ability];
        if (currentWeather === "Souffle Delta") {
            if (lastAutoWeather.indexOf("Souffle Delta") === -1) {
                newWeather = lastAutoWeather[i];
            }
        } else if (primalWeather.indexOf(currentWeather) > -1) {
            if (lastAutoWeather[i] === "Souffle Delta" || primalWeather.indexOf(lastAutoWeather[i]) > -1) {
                newWeather = lastAutoWeather[i];
    ...
    if (newWeather === "Souffle Delta" || primalWeather.indexOf(newWeather) > -1) {
        ...
        for (var k = 0; k < $("input:radio[name='weather']").length; k++) {
            ...
            if (primalWeather.indexOf(val) === -1 && val !== "Souffle Delta") {
...
var lastManualStatus = {"#p1":"Bonne santé", "#p2":"Bonne santé"};
var lastAutoStatus = {"#p1":"Bonne santé", "#p2":"Bonne santé"};
function autosetStatus(p, item) {
    ...
    if (item === "Orbe Flamme") {
        lastAutoStatus[p] = "Brûlé";
        $(p + " .status").val("Brûlé");
        $(p + " .status").change();
    } else if (item === "Orbe Toxique") {
        lastAutoStatus[p] = "Grâvement empoisonné";
        $(p + " .status").val("Grâvement empoisonné");
        $(p + " .status").change();
    } else {
        lastAutoStatus[p] = "Bonne santé";
...
$(".status").bind("keyup change", function() {
    if ($(this).val() === 'Grâvement empoisonné') {
...
$(".move-selector").change(function() {
    ...
    var move = moves[moveName] || moves['(Sans Attaque)'];
    ...
    if (move.isMultiHit) {
        ...
        moveGroupObj.children(".move-hits").val($(this).closest(".poke-info").find(".ability").val() === 'Multi-Coups' ? 5 : 3);
    } else {
...
$(".set-selector").change(function() {
    ...
    if (pokemon) {
        ...
        pokeObj.find(".status").val("Bonne santé");
        $(".status").change();
        var moveObj;
        ...
        if (pokemonName in setdex && setName in setdex[pokemonName]) {
            ...
            setSelectValueIfValid(pokeObj.find(".nature"), set.nature, "Hardi");
            setSelectValueIfValid(abilityObj, pokemon.ab ? pokemon.ab : set.ability, "");
            setSelectValueIfValid(itemObj, set.item, "");
            for (i = 0; i < 4; i++) {
                moveObj = pokeObj.find(".move" + (i+1) + " select.move-selector");
                setSelectValueIfValid(moveObj, set.moves[i], "(Sans Attaque)");
                moveObj.change();
            }
        } else {
            ...
            pokeObj.find(".nature").val("Hardi");
            setSelectValueIfValid(abilityObj, pokemon.ab, "");
            itemObj.val("");
            for (i = 0; i < 4; i++) {
                moveObj = pokeObj.find(".move" + (i+1) + " select.move-selector");
                moveObj.val("(Sans Attaque)");
                moveObj.change();
            }
        }
...
function showFormes(formeObj, setName, pokemonName, pokemon) {
    ...
    if (setName !== 'Set blanc') {
        ...
        if ((set.item.indexOf('ite') !== -1 && set.item.indexOf('ite Y') === -1) ||
            (pokemonName === "Groudon" && set.item.indexOf("Gemme Rouge") !== -1) ||
            (pokemonName === "Kyogre" && set.item.indexOf("Gemme Bleue") !== -1) ||
            (pokemonName === "Meloetta" && set.moves.indexOf("Chant Antique") !== -1) ||
            (pokemonName === "Rayquaza" && set.moves.indexOf("Draco Ascension") !== -1)) {
            defaultForme = 1;
...
$(".forme").change(function() {
    ...
    } else if (setName !== "Set blanc" && abilities.indexOf(setdex[pokemonName][setName].ability) > -1) {
    ...
    if (pokemonName === "Darumacho") {
        container.find(".percent-hp").val($(this).val() === "Darumacho-Z" ? "50" : "100").keyup();
    }
});
...
function getTerrainEffects() {
    ...
    switch (className) {
        ...
        case "item":
            ...
            if (terrainValue === "Électrik") {
                $("#" + id).find("[value='Endormi']").prop("disabled", isGrounded($("#" + id)));
            } else if (terrainValue === "Brumeux") {
                $("#" + id).find(".status").prop("disabled", isGrounded($("#" + id)));
            }
            break;
        default:
            $("input:checkbox[name='terrain']").not(this).prop("checked", false);
            if ($(this).prop("checked") && $(this).val() === "Électrik") {
                $("#p1").find("[value='Endormi']").prop("disabled", isGrounded($("#p1")));
                $("#p2").find("[value='Endormi']").prop("disabled", isGrounded($("#p2")));
                ...
                $("#p1").find("[value='Endormi']").prop("disabled", false);
                $("#p1").find(".status").prop("disabled", false);
                $("#p2").find("[value='Endormi']").prop("disabled", false);
...
function isGrounded(pokeInfo) {
    return $("#gravity").prop("checked") || (pokeInfo.find(".type1").val() !== "Vol" && pokeInfo.find(".type2").val() !== "Vol" &&
            pokeInfo.find(".ability").val() !== "Lévitation" && pokeInfo.find(".item").val() !== "Ballon");
}
...
function calculate() {
    ...
    for (var i = 0; i < 4; i++) {
        ...
        result.koChanceText = p1.moves[i].bp === 0 ? 'joli coup'
                : getKOChanceText(result.damage, p1.moves[i], p2, field.getSide(1), p1.ability === 'Mauvais rêves');
        if(p1.moves[i].isMLG){
            result.koChanceText = "<a href = 'https://www.youtube.com/watch?v=iD92h-M474g'>it's a one-hit KO!</a>"; //dank memes
        }
        $(resultLocations[0][i].move + " + label").text(p1.moves[i].name.replace("Puissance Cachée", "PV"));
        ...
        result.damageText = minDamage + "-" + maxDamage + " (" + minPercent + " - " + maxPercent + "%)";
        result.koChanceText = p2.moves[i].bp === 0 ? 'joli coup'
                : getKOChanceText(result.damage, p2.moves[i], p1, field.getSide(0), p2.ability === 'Mauvais rêves');
        if(p2.moves[i].isMLG){
            result.koChanceText = "<a href = 'https://www.youtube.com/watch?v=iD92h-M474g'>it's a one-hit KO!</a>";
        }
        $(resultLocations[1][i].move + " + label").text(p2.moves[i].name.replace("Puissance Cachée", "PV"));
    ...
    bestResult.change();
    $("#resultHeaderL").text("Capacité du 1er Pokémon: " + p1.name);
    $("#resultHeaderR").text("Capacité du 2ème Pokémon: " + p2.name);
}
    ...
    if (damageResults) {
        ...
        if (result) {
            ...
            if (result.parentDamage) {
                $("#damageValues").text("(Premier coup: " + result.parentDamage.join(", ") + 
                    "; Second coup: " + result.childDamage.join(", ") + ")");
            } else {
...
function Pokemon(pokeInfo) {
    ...
    this.toxicCounter = this.status === 'Grâvement empoisonné' ? ~~pokeInfo.find(".toxic-counter").val() : 0;
    this.moves = [
...
$(".gen").change(function () {
    ...
    $("select.type2").find("option").remove().end().append("<option value=\"\">(aucun)</option>" + typeOptions);
    var moveOptions = getSelectOptions(Object.keys(moves), true);
    $("select.move-selector").find("option").remove().end().append(moveOptions);
    var abilityOptions = getSelectOptions(abilities, true);
    $("select.ability").find("option").remove().end().append("<option value=\"\">(autre)</option>" + abilityOptions);
    var itemOptions = getSelectOptions(items, true);
    $("select.item").find("option").remove().end().append("<option value=\"\">(autre)</option>" + itemOptions);
...
function getSetOptions() {
    ...
    for (var i = 0; i < pokeNames.length; i++) {
        ...
        setOptions.push({
            pokemon: pokeName,
            set: "Set blanc",
            text: pokeName + " (Set blanc)",
            id: pokeName + " (Set blanc)"
        });
    }
    return setOptions;
}
